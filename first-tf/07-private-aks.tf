resource "azurerm_kubernetes_cluster" "aks-test-first" {
  ...
  default_node_pool {
    ...
    vnet_subnet_id = azurerm_subnet.snet-adlosrfirst-001-aks.id
  }
  #   microsoft_defender {
...
resource "azurerm_kubernetes_cluster_node_pool" "user" {
  ...
  name                  = "user"
  vnet_subnet_id        = azurerm_subnet.snet-adlosrfirst-001-aks.id
  kubernetes_cluster_id = azurerm_kubernetes_cluster.aks-test-first.id
  vm_size               = "Standard_B2als_v2"
  node_count            = 1
  os_type               = "Linux"
  priority              = "Regular"
